...
但同時也是缺點：只限於有寫的文章或頁面才能使用發 LINE 的功能 function。
...
1. 選一個你要寫的地方，以忍者 Blog 來說，你可以發一篇文章或一個頁面。
...
~~~
3. 關於這部分的 code，除了必要功能外，有很多很關於外觀設定。
...
1. ``<head>`` 的部分貼上 [設置主要功能](#!teach/line.post.md#設置主要功能) 裡面的 code，再修改成你的版本。
主要請你要設置你自己的 Zapier 接收網址。
2. ``<body>`` 或一般發布文章的地方，在你想創表單介面的地方。
貼上 [設置表單操作介面選項](#!teach/line.post.md#設置表單操作介面選項) 裡面的 code。
也是可以修改，但修改這邊主要是影響視覺效果。
要更改發送目標的話，請去研究 [設置主要功能](#!teach/line.post.md#設置主要功能) 裡面的 code。
3. 所以 code 全都可以改。
只要重點的部分不要改壞掉都會有功能。
4. 要有防手殘的自動記憶輸入內容功能，不一定要寫在 ``window.onload = function ()`` 裡面。
有些情況可能要請各位站長自己變化。
例如我下面裝的，就是沒有寫在那裡面。
只有單純安裝更裡面的重點：
~~~
...
- 當然這種玩法不只 LINE 可以玩。
其他的平台只要有開放 API 接收訊息傳遞的功能，也一樣可以這樣玩。
但你必須研究一下對方的寫法、接法還有相關參數跟限制。
我之所以要透過 <Zapier.com> 來傳遞主要是利用他來整理輸出的資料格式跟附加 head 資訊。
如果獨立自己去加 head 會有點麻煩。需要透過伺服器之類的。
  - 當然你也可以寫一個傳統會寄電子郵件的表單，然後郵件目標寫 <Trello.com> 的或是 <ifttt.com> 的信箱。
  如果是寫 <ifttt.com> 的信箱，你還可以看你要傳到哪個平台又要怎麼連動。全都看你個人人的想法去玩玩看。
